! function(a) {
    ...
    var a = function() { beta = this, beta.init() };
    a.prototype = {
        init: function() { beta.initInstragram(), beta.initFilter(), beta.initCalendar(), beta.initRevolutionSlider(), beta.initRoundabout(), beta.initSlider(), beta.initHistorySlider(), beta.initCircliful(), beta.initMedia(), beta.initTooltip(), beta.initProgressBar(), beta.initColorbox(), beta.onCart(), beta.setSelect(), beta.animateBanners(), beta.absFullwidth(), beta.filterItems(), beta.accordionHandler() },
        initInstragram: function() { dug({ endpoint: "https://api.instagram.com/v1/users/1140781/media/recent/?client_id=01a35e4dd4bf4d2681e48c88d006636e&count=6", target: jQuery("#beta-instagram-feed")[0], cacheExpire: 3600, template: '' }) },
        initFilter: function() { jQuery.fn.slider && (jQuery("#price-filter-range").slider({ range: !0, min: 0, max: 1e3, values: [0, 300], slide: function(a, b) { jQuery("#price-filter-amount").text("$" + b.values[0] + " - $" + b.values[1]) } }), jQuery("#price-filter-amount").text("$" + jQuery("#price-filter-range").slider("values", 0) + " - $" + jQuery("#price-filter-range").slider("values", 1))) },
        initCalendar: function() { jQuery.fn.datepicker && jQuery(".beta-calendar").datepicker() },
        initRevolutionSlider: function() {
            if (jQuery.fn.revolution) {
                var a = { delay: 9e3, startwidth: 1140, startheight: 480, navigationType: "none" };
                jQuery(".tp-banner").hasClass("fullwidth") && (a.fullWidth = "on", a.forceFullWidth = "on", a.startheight = 530), jQuery(".tp-banner").revolution(a)
            }
        },
        initRoundabout: function() { jQuery.fn.roundabout && jQuery(".beta-roundabout").roundabout({ responsive: !0 }) },
        initCircliful: function() {
            if (jQuery.fn.circliful) {
                ...
                jQuery(".beta-rotator").circliful(c.matches ? { dimension: "370px" } : b.matches ? { dimension: "158px" } : a.matches ? { dimension: "213px" } : { dimension: "260px" })
            }
        },
        initMedia: function() {
            if (jQuery.fn.mediaelementplayer) {
                var a = jQuery(".beta-audio").parent().width();
                jQuery("audio,video").mediaelementplayer({ audioWidth: a })
            }
        },
        initTooltip: function() { jQuery("[data-toggle='tooltip']").tooltip() },
        initProgressBar: function() {
            jQuery(".progress-bar").each(function(a, b) {
                var c = jQuery(b).attr("aria-valuenow"),
                    d = parseInt(c) / 100 * 5e3;
                jQuery(b).animate({ width: c + "%" }, d)
            })
        },
        initSlider: function() {
            jQuery(".beta-slider").each(function(a, b) {
                var c = jQuery(b),
                    ...
                    i = { auto: !0, autoHover: !0, controls: !1, pager: !1 };
                g.length > 0 && (i.pager = !0, i.pagerCustom = g), h.length > 0 && (i.pager = !0, i.pagerCustom = h);
                var j = d.bxSlider(i);
                e.on("click", function() { return j.goToNextSlide(), !1 }), f.on("click", function() { return j.goToPrevSlide(), !1 })
            })
        },
        initHistorySlider: function() {
            jQuery(".history-slider").each(function(a, b) {
                ...
                d.bxSlider({ auto: !0, autoHover: !0, controls: !1, pager: !0, pagerCustom: e })
            })
        },
        initColorbox: function() { jQuery(".colorbox").colorbox({ maxWidth: "95%", maxHeight: "95%", rel: "gal" }) },
        setSelect: function() { jQuery("select[name='languages']").betaSelect({ text: "Language:", value: "English" }), jQuery("select[name='currency']").betaSelect({ text: "Currency:", value: "USD" }), jQuery(".beta-select-primary").betaSelect({ value: "Select", "class": "beta-select-primary" }) },
        onCart: function() {
            jQuery(".cart .beta-select").on("click", function() { return jQuery(".cart-body").slideToggle(), !1 });
            var a = jQuery(".cart");
            jQuery(document).mouseup(function(b) { a.is(b.target) || 0 !== a.has(b.target).length || jQuery(".cart-body").slideUp() })
        },
        ...
        absFullwidth: function() {
            jQuery(".abs-fullwidth").each(function(a, b) {
                var c = jQuery(b).height(),
                    d = jQuery(b).find("img");
                jQuery(b).after("<div class='abs-fullwidth-height' style='height:" + c + "px'></div>"), d.length > 0 && d.on("load", function() { c = jQuery(b).height(), jQuery(b).next(".abs-fullwidth-height").height(c + "px") })
            })
        },
    ...
    var b = {
        ...
        initTabs: function() {
            ...
            }), jQuery("a.woocommerce-review-link").click(function() { return jQuery(".reviews_tab a").click(), !0 })
        }
    };
    jQuery(function() {
        new a;
        "undefined" != typeof BetaUtils && BetaUtils.setMap(jQuery("#beta-map")[0], { latitude: 40.722803, longitude: -74.00882, zoom: 10, content: "BetaDesign", title: "Beta Design" }), b.init(), jQuery(".shipping-calculator-button").on("click", function() { return jQuery(".shipping-calculator-form ").slideToggle(), !1 }), jQuery("input[name='payment_method']").on("click", function() {
            ...
            jQuery(".payment_box").each(function(b, c) { jQuery(c).hasClass(a) ? jQuery(c).show() : jQuery(c).hide() })
        }), jQuery(".beta-menu-toggle").on("click", function() { return jQuery(".main-menu > ul").slideToggle(), !1 });
